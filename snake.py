...
def coin():
    return random.randint(0, screen_width - snake_size), random.randint(0, screen_height - snake_size)
...
while running:
    ...
    if pygame.Rect(snake_x, snake_y, snake_size, snake_size).colliderect(pygame.Rect(coin_x, coin_y, snake_size, snake_size)):
        score += 1
        coin_x, coin_y = coin()

    if green_coin_visible: 
        pygame.draw.rect(screen, "green", (green_coin_x, green_coin_y, snake_size, snake_size))

    if not green_coin_visible and pygame.time.get_ticks() - green_coin_timer > 3000: #kdyz neni visible coin a cas je pod 3 s tak se spawne coin a je videt, funkce se opakuje az po sebrani (v collide se nastavi visible false)
        green_coin_x, green_coin_y = green_coin()
        green_coin_visible = True

    if green_coin_visible and pygame.Rect(snake_x, snake_y, snake_size, snake_size).colliderect(pygame.Rect(green_coin_x, green_coin_y, snake_size, snake_size)):
        score += 100
